...
def initCircularList():
    global window, canvas, inpData, lbl_Error, inpDelNode

    window = Tk()
    window.title("Visual Circular Linked List")
    ...
    Label(
        ...
        padx=15,
        font=(fontName, 10),
        text="• Circular Linked List is a variation of Linked list in which the first element points\n  to the last element and the last element points to the first element. ",
        justify=LEFT
    ).pack(anchor=W)
    ...
    Label(
        ...
        padx=15,
        font=(fontName, 10),
        text="• Why Circular? In a singly linked list, for accessing any node of linked list, we start traversing from the first node.",
        justify=LEFT
    ).pack(anchor=W)
    ...
    Label(
        ...
        padx=15,
        font=(fontName, 10),
        text="• If we are at any node in the middle of the list, then it is not possible to access nodes that precede the given node.",
        justify=LEFT
    ).pack(anchor=W)
...
def lblHead():
    ...
    try:
        if arr_Nodes[0]:
            ...
            head_arrow.append(canvas.create_line(170, 220, 170, 330, arrow=FIRST))
    except IndexError:
        print("")
...
def endNextArrow(x = 270):
    global arr_endArrow

    try:
        canvas.delete(arr_endArrow[0][0])
        canvas.delete(arr_endArrow[0][1])
        canvas.delete(arr_endArrow[0][2])
        canvas.delete(arr_endArrow[0][3])
        arr_endArrow.pop(0)
    except IndexError:
        print()

    arrow = []
    arrow.append(canvas.create_line(200, 220, 200, 280, arrow=FIRST)) # | Small with Arrow
    arrow.append(canvas.create_line(200, 280, x, 280)) # --- Big
    arrow.append(canvas.create_line(x, 125, x, 280)) # | Big
    arrow.append(canvas.create_line(x - 40, 125, x, 125)) # -- Small

    arr_endArrow.append(arrow)
...
def insertAtEnd():
    ...
    if getInpData():
        ...
        arr_x1.append(x1)
        if index > 0:
            firstNode = arr_Nodes[0]

            newNode = visualNode(getInpData(),"Null",index)
            setItemText(newNode[1][1],getItemText(firstNode[2]))
            ...
            newNode = visualNode(getInpData(),"Null",index)
            setItemText(newNode[1][1],getItemText(newNode[2]))
            arr_Nodes.append(newNode)
...
def insertAtFront():
    ...
    if getInpData():
        ...
        if index >= 0:
            ...
            setItemText(lastNode[1][1],getItemText(newNode[2]))
                ...
                try:
                    if int(getDelNode()) == index:
                        try:
                            ...
                            firstNode = arr_Nodes[0]
                            setItemText(prNode[1][1],getItemText(firstNode[2]))
                        ...
                        firstNode = arr_Nodes[0]
                        lastNode = arr_Nodes[len(arr_Nodes) - 1]
                        setItemText(lastNode[1][1],getItemText(firstNode[2]))
